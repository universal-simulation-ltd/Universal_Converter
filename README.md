...
| **Images** | **WebP**, **JPEG**, **PNG**, **AVIF**, **GIF** | The browser's own canvas encoder — convert, re-quality and resize. Every row is **priced before it runs** (`≈ 15.2 MB · 12× bigger`) by encoding a 3×3 mosaic of full-resolution crops and multiplying by the pixel count — measured at 0.81×–1.22× of the real conversion across photos, graphics and screenshots. **Clicking another format puts a finished row back in the queue**, so trying PNG against JPEG is one click rather than a re-drop. **HEIC/HEIF in** (an iPhone photo → PNG or JPEG) is the one exception to "no decoder ships with this app": nothing outside Safari reads HEIC, so [`heic-to`](https://www.npmjs.com/package/heic-to) (libheif 1.19) is dynamically imported the first time one is dropped, and never otherwise. ⚠️ Not `heic2any`: it decoded every HEIC *we* generated and failed every real photo off an iPhone, because an iPhone writes a `grid` of HEVC tiles and its 2020-era libheif does not read one. **GIF is the second exception**, and the only target here the browser will not write at all — there is no `toBlob('image/gif')` in any engine — so the palette, the LZW and the file are ours (`gif.ts`). An **animated GIF in gives an animated GIF out**, every frame, with its delays and loop count carried across; that needs a GIF *reader* too (`gifdecode.ts`), because `createImageBitmap` hands back frame one of an animation and says nothing about the rest. Choosing a single-frame format with an animation in the queue says so in amber before you press Convert, rather than quietly returning a still |
...
| Animated GIF | Ours end to end — `gifdecode.ts` reads it, `gif.ts` writes it. Byte-identical copies of Universal Compress's pair; keep them that way |
